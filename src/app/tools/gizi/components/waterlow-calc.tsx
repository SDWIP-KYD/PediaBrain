...
export function WaterlowCalc() {
  ...
  const [ibwManual, setIbwManual] = useState(0);
  ...
  const useIbw = ibwManual > 0 ? ibwManual : ibwFH;
  const pctWfH = Math.round((weight / useIbw) * 100 * 10) / 10;
  const pctHfA = Math.round((height / mH) * 100 * 10) / 10;

  let wasting = "";
  let wastingColor = "";
  if (pctWfH >= 90) { wasting = "Normal"; wastingColor = "text-green-400"; }
  else if (pctWfH >= 80) { wasting = "Ringan (80-89%)"; wastingColor = "text-yellow-400"; }
  else if (pctWfH >= 70) { wasting = "Sedang (70-79%)"; wastingColor = "text-orange-400"; }
  else { wasting = "Berat (<70%)"; wastingColor = "text-red-500"; }

  let stunting = "";
  let stuntingColor = "";
  if (pctHfA >= 95) { stunting = "Normal"; stuntingColor = "text-green-400"; }
  else if (pctHfA >= 90) { stunting = "Ringan (90-94%)"; stuntingColor = "text-yellow-400"; }
  else if (pctHfA >= 85) { stunting = "Sedang (85-89%)"; stuntingColor = "text-orange-400"; }
  else { stunting = "Berat (<85%)"; stuntingColor = "text-red-500"; }

  const notes = [
    pctWfH < 80 && pctHfA < 90 ? "Malnutrisi campuran (wasting + stunting)" : null,
  ].filter(Boolean).join(". ");

  return (
    <CalcCard title="Klasifikasi Waterlow" subtitle="Wasting & Stunting — % median WHO" icon="📐" color="blue">
      <div className="space-y-3">
        <InfoBox>
          <strong>Waterlow:</strong> Membedakan wasting (kurus akut) vs stunting (pendek kronik). Diperlukan BB/TB dan TB/U.
        </InfoBox>
        <div className="grid grid-cols-3 gap-2">
          ...
          <CalcInput label="TB Aktual (cm)" unit="cm" value={height} onChange={(v) => setHeight(typeof v === "string" ? parseFloat(v) || 0 : v)} min={30} step={0.5} />
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <CalcInput label="BB ideal untuk TB (kg)" unit="kg" value={ibwManual} onChange={(v) => setIbwManual(typeof v === "string" ? parseFloat(v) || 0 : v)} min={0} step={0.1} placeholder={`${ibwFH} (otomatis)`} />
        </div>
        <CalcResult color="blue">
          <ResultGrid cols={2}>
            <ResultItem label="BB/TB (% median)" value={`${pctWfH}%`} />
            <ResultItem label="Wasting" value={wasting} className={wastingColor} />
            <ResultItem label="TB/U (% median)" value={`${pctHfA}%`} />
            <ResultItem label="Stunting" value={stunting} className={stuntingColor} />
          </ResultGrid>
        </CalcResult>
        {notes && (
          <InfoBox>
